...
class LinkFilter {
    ...
    filter(links, criteria) {
        ...
        if (criteria.excludeDomains && criteria.excludeDomains.length > 0) {
            filtered = filtered.filter(link => {
                try {
                    ...
                    return !criteria.excludeDomains.some(domain => hostname === domain.toLowerCase() ||
        ...
        if (criteria.urlPattern) {
            filtered = filtered.filter(link => this.matchesPattern(link.url, criteria.urlPattern));
        }
        ...
        if (criteria.protocols && criteria.protocols.length > 0) {
            ...
            filtered = filtered.filter(link => normalizedProtocols.includes(link.protocol.toLowerCase()));
        }
        return filtered;
    }
    /**
    ...
    filterByDomain(links, domains) {
        ...
        return links.filter(link => {
            try {
                ...
                return normalizedDomains.some(domain => hostname === domain || hostname.endsWith('.' + domain));
            }
            catch {
...
//# sourceMappingURL=LinkFilter.js.map