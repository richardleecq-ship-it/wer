 ...
 */
async function runCLI(url: string | undefined, rawOptions: any): Promise<void> {
  ...
  if (cliOptions.batch) {
    ...
    throw new ValidationError(
      'Either a URL or --batch option must be provided',
      'url',
      'URL or batch file required'
    );
  }
}
 ...
 */
function parseCLIOptions(url: string | undefined, rawOptions: any): CLIOptions {
  const cliOptions: CLIOptions = {
    url,
    batch: rawOptions.batch,
    format: rawOptions.format as OutputFormat,
  ...
  if (rawOptions.domains) {
    cliOptions.domains = rawOptions.domains.split(',').map((d: string) => d.trim());
  }
  
  if (rawOptions.excludeDomains) {
    cliOptions.excludeDomains = rawOptions.excludeDomains.split(',').map((d: string) => d.trim());
  }
  
  if (rawOptions.protocols) {
    cliOptions.protocols = rawOptions.protocols.split(',').map((p: string) => p.trim());
  }
...
function validateOptions(options: CLIOptions): void {
  // Validate format
  const validFormats: OutputFormat[] = ['json', 'csv', 'markdown', 'text'];
  if (options.format && !validFormats.includes(options.format)) {
    throw new ValidationError(
      `Invalid format: ${options.format}. Must be one of: ${validFormats.join(', ')}`,
      'format',
      'Must be json, csv, markdown, or text'
    );
  }
  ...
  if (options.internalOnly && options.externalOnly) {
    throw new ValidationError(
      'Cannot use both --internal-only and --external-only',
      'filter',
 ...
 */
async function executeSingleMode(options: CLIOptions): Promise<void> {
 ...
 */
async function executeBatchMode(options: CLIOptions): Promise<void> {
 ...
 */
function buildExtractionOptions(options: CLIOptions): ExtractionOptions {
 ...
 */
async function displayResults(result: ExtractionResult, options: CLIOptions): Promise<void> {
  // Format the result
 ...
 */
async function displayBatchResults(result: BatchResult, options: CLIOptions): Promise<void> {
 ...
 */
function displaySummary(result: ExtractionResult, options: CLIOptions): void {
 ...
 */
function displayBatchSummary(result: BatchResult, options: CLIOptions): void {
  console.log('\n' + '═'.repeat(80));
